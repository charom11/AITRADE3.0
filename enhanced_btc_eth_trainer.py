#!/usr/bin/env python3
"""
Enhanced BTCUSDT and ETHUSDT Model Trainer
Specialized training system with emphasis on BTCUSDT and ETHUSDT performance optimization.
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('enhanced_btc_eth_training.log'),
...
class EnhancedTrainingConfig:
    """Configuration for enhanced BTC/ETH training"""
...
class EnhancedBTCETHTrainer:
    """Enhanced trainer specifically optimized for BTCUSDT and ETHUSDT"""
    ...
    def __init__(self, config: EnhancedTrainingConfig):
        ...
        logger.info(f"Enhanced BTC/ETH Trainer initialized for symbols: {self.config.priority_symbols}")
    
    def fetch_enhanced_data(self, symbol: str, limit: int = None) -> pd.DataFrame:
        """Fetch enhanced market data with additional features"""
        try:
            if limit is None:
                limit = self.config.data_limit
            
            logger.info(f"Fetching enhanced data for {symbol} ({limit} candles)")
            ...
            try:
                # Try to get funding rate for futures
            ...
            df['market_regime'] = np.where(df['trend_strength'] > 1.5, 'trending', 
                                         np.where(df['volatility_ratio'] > 1.2, 'volatile', 'ranging'))
            
            # Clean data
            df.dropna(inplace=True)
            
            logger.info(f"Enhanced data fetched for {symbol}: {len(df)} samples")
            return df
    ...
    def prepare_enhanced_training_data(self, data: pd.DataFrame, symbol: str) -> Tuple[pd.DataFrame, pd.Series]:
        ...
        try:
            ...
            target_1h = np.where(adjusted_returns_1h > 0.5, 1, 
                               np.where(adjusted_returns_1h < -0.5, 0, -1))
            target_4h = np.where(adjusted_returns_4h > 1.0, 1, 
                               np.where(adjusted_returns_4h < -1.0, 0, -1))
            target_24h = np.where(adjusted_returns_24h > 2.0, 1, 
            ...
            final_target = np.where(combined_target > 0.3, 1, 
    ...
    def train_enhanced_models(self, symbol: str, data: pd.DataFrame) -> Dict[str, Dict]:
        ...
        try:
            ...
            models_config = {
                ...
                'neural_network': {
                    'model': MLPClassifier(random_state=self.config.random_state, max_iter=500),
                    'params': {
    ...
    def _create_ensemble_model(self, symbol: str, individual_results: Dict, X: np.ndarray, y: pd.Series) -> Dict:
        ...
        try:
            ...
            ensemble_pred_proba = np.zeros((X.shape[0], 3))  # 3 classes: -1, 0, 1
...
def main():
    """Main function to run enhanced BTC/ETH training"""
    print("🚀 Enhanced BTC/ETH Model Trainer")
    print("="*50)
    ...
    # Initialize trainer
    trainer = EnhancedBTCETHTrainer(config)
